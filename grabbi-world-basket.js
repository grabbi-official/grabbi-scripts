(function() {
  ...
  drawer.innerHTML = '<div id="grb-head"><div id="grb-head-title">🛒 Your Basket</div><button id="grb-head-close" aria-label="Close basket">✕</button></div><div id="grb-loyalty">⭐ Earning <strong id="grb-pts">0 points</strong> on this order &bull; 1pt per £1</div><div id="grb-bar"><div id="grb-bar-track"><div id="grb-bar-fill"></div></div><div id="grb-bar-msg">Add items to get started 🛒</div></div><div id="grb-items"><div id="grb-empty">Your basket is empty.<br>Browse our range and add items!</div></div><div id="grb-foot"><div id="grb-total-row"><span id="grb-total-label">Total</span><span id="grb-total-val">£0.00</span></div><button id="grb-checkout">Proceed to Checkout →</button></div>';
  ...
  // Open / close
  window.grbOpen = function() { grbRender(); overlay.classList.add('open'); drawer.classList.add('open'); };
  window.grbClose = function() { overlay.classList.remove('open'); drawer.classList.remove('open'); };

  // ── ADD ITEM — defined globally so ANY page can call addToBasket() ──
  ...
  // ── addToBasket — defined globally so pages don't need their own script ──
  window.addToBasket = function(card, name, brand, price) {
    ...
    if (btn) {
      ...
      setTimeout(function(){ btn.textContent = '+'; btn.classList.remove('added'); }, 900);
    }
    window.grbAdd(name, brand, price);
  };

  // ── toggleAcc — defined globally so pages don't need their own script ──
  window.toggleAcc = function(el) {
    var body = el.nextElementSibling;
    var isOpen = body.classList.contains('open');
    document.querySelectorAll('.acc-body').forEach(function(b){ b.classList.remove('open'); });
    document.querySelectorAll('.acc-header').forEach(function(b){ b.classList.remove('open'); });
    if (!isOpen) { body.classList.add('open'); el.classList.add('open'); }
  };

  // ── scrollToShop — defined globally ──
  ...
  function grbRender() {
    ...
    itemsEl.innerHTML = items.map(function(e) {
      ...
      return '<div class="grb-line"><div class="grb-line-info">' + n + '<span class="grb-line-brand">' + v.brand + '</span></div><div class="grb-qty"><button class="grb-qty-btn" onclick="grbQty(\'' + safe + '\',-1)">−</button><span style="font-weight:700;font-size:14px;min-width:16px;text-align:center">' + v.qty + '</span><button class="grb-qty-btn" onclick="grbQty(\'' + safe + '\',1)">+</button></div><div class="grb-line-price">£' + (v.price * v.qty).toFixed(2) + '</div></div>';
    }).join('');
  }
